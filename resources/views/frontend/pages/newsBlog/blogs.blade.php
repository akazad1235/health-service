...
<head>
	...
	<title>​News &amp; Blog | 1st Choice Family Service</title>
	<script>
		...
		<main id="main-content">
            <section class="banner solid-blue">
                <div class="frame row grid-row">
                    ...
                    <div class="col-lg-7">
                        <h1 class="hide-logo-trigger">News & Blogs</h1>
                    </div>
                </div>
            </section>
            ...
            <div class="landing-archive grid-row">
                ...
                @foreach ($allBlog as $item)
                    <div class="post-item">
                        <img  src="{{asset('../backend/img/blog/'.$item->image)}}">
                        <div class="post-item-inner">
                            <a href="{{route('blog.single', base64_encode($item->id))}}"
                                class="post-item-title">{{$item->title}}
                            </a>
                            <div class="post-item-teaser">{!! substr($item->desc, 0,150) !!}</div>
                            <a class="link"
                                href="{{route('blog.single', base64_encode($item->id))}}">Learn
                                More</a>
                        </div>
                    </div>
                @endforeach
